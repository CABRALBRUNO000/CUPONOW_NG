...
var ImagePromptTemplate = class _ImagePromptTemplate extends BasePromptTemplate {
  ...
  partial(values) {
    return __async(this, null, function* () {
      ...
      const newPartialVariables = __spreadValues(__spreadValues({}, this.partialVariables ?? {}), values);
      const promptDict = __spreadProps(__spreadValues({}, this), {
  ...
  formatPromptValue(values) {
    return __async(this, null, function* () {
      const formattedPrompt = yield this.format(values);
...
var BaseMessagePromptTemplate = class extends Runnable {
  ...
  invoke(input, options) {
    return __async(this, null, function* () {
      return this._callWithConfig((input2) => this.formatMessages(input2), input, __spreadProps(__spreadValues({}, options), {
        runType: "prompt"
      }));
    });
  }
...
var BaseMessageStringPromptTemplate = class extends BaseMessagePromptTemplate {
  ...
  formatMessages(values) {
    return __async(this, null, function* () {
      return [yield this.format(values)];
    });
  }
};
var BaseChatPromptTemplate = class extends BasePromptTemplate {
  ...
  format(values) {
    return __async(this, null, function* () {
      return (yield this.formatPromptValue(values)).toString();
    });
  }
  formatPromptValue(values) {
    return __async(this, null, function* () {
      const resultMessages = yield this.formatMessages(values);
...
var ChatMessagePromptTemplate = class extends BaseMessageStringPromptTemplate {
  ...
  format(values) {
    return __async(this, null, function* () {
      return new ChatMessage(yield this.prompt.format(values), this.role);
    });
  }
  ...
  static fromTemplate(template, additionalOptions) {
    ...
    for (const item of template) {
      if (typeof item === "string" || typeof item === "object" && "text" in item) {
        ...
        const options = __spreadValues(__spreadValues({}, additionalOptions), typeof item !== "string" ? {
  ...
  format(input) {
    return __async(this, null, function* () {
      if (this.prompt instanceof BaseStringPromptTemplate) {
        const text = yield this.prompt.format(input);
        ...
        for (const prompt of this.prompt) {
          ...
          if (prompt instanceof BaseStringPromptTemplate) {
            ...
            content.push(__spreadProps(__spreadValues({}, additionalContentFields), {
              type: "text",
              text: formatted
            }));
...
var ChatPromptTemplate = class _ChatPromptTemplate extends BaseChatPromptTemplate {
  ...
  _parseImagePrompts(message, inputValues) {
    return __async(this, null, function* () {
      ...
      const formattedMessageContent = yield Promise.all(message.content.map((item) => __async(this, null, function* () {
    ...
    return __async(this, null, function* () {
      ...
      for (const promptMessage of this.promptMessages) {
        if (promptMessage instanceof BaseMessage) {
          resultMessages.push(yield this._parseImagePrompts(promptMessage, allValues));
        } else {
  ...
  static fromMessages(promptMessages, extra) {
    const flattenedMessages = promptMessages.reduce((acc, promptMessage) => acc.concat(
      ...
      promptMessage instanceof _ChatPromptTemplate ? promptMessage.promptMessages : [_coerceMessagePromptTemplateLike(promptMessage, extra)]
    ), []);
    ...
    return new this(__spreadProps(__spreadValues({}, extra), {
...
var FewShotPromptTemplate = class _FewShotPromptTemplate extends BaseStringPromptTemplate {
  ...
  format(values) {
    return __async(this, null, function* () {
      const allValues = yield this.mergePartialAndUserVariables(values);
      const examples = yield this.getExamples(allValues);
      const exampleStrings = yield Promise.all(examples.map((example) => this.examplePrompt.format(example)));
...
var FewShotChatMessagePromptTemplate = class _FewShotChatMessagePromptTemplate extends BaseChatPromptTemplate {
  ...
  formatMessages(values) {
    return __async(this, null, function* () {
      const allValues = yield this.mergePartialAndUserVariables(values);
      let examples = yield this.getExamples(allValues);
      ...
      for (const example of examples) {
        const exampleMessages = yield this.examplePrompt.formatMessages(example);
  ...
  format(values) {
    return __async(this, null, function* () {
      const allValues = yield this.mergePartialAndUserVariables(values);
      const examples = yield this.getExamples(allValues);
      const exampleMessages = yield Promise.all(examples.map((example) => this.examplePrompt.formatMessages(example)));
...
//# sourceMappingURL=chunk-VP7I2KXP.js.map
